...
def vulnerable_position(gamestate, check_x, check_y, player, opponent):
    return vuln_pos_left(gamestate, check_x, check_y, player, opponent) or vuln_pos_right(gamestate, check_x, check_y, player,
                                                                               opponent) or vuln_pos_top(gamestate,
...
def value_my_pieces(gamestate, player, opponent):
    ...
    for piece in gamestate.players[player].pieces.values():
        if vulnerable_position(piece.get_position()[0], piece.get_position()[1], player, opponent):
            continue
        elif piece.evolved:
            value_counter += 1000
        else:
...
def value_opponents_pieces(gamestate, player, opponent):
    ...
    for piece in gamestate.players[opponent].pieces.values():
        if vulnerable_position(piece.get_position()[0], piece.get_position()[1], opponent, player):
            continue
        elif piece.evolved:
            value_counter -= 1000
        else:
